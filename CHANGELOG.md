...
- **Antigravity hook support** - The same binary handles [Antigravity](https://antigravity.google/docs/hooks) (the `agy` CLI) via a `clojure-mcp-light.antigravity` adapter, opted into with `--antigravity --event <Name>` (Antigravity's camelCase payload carries no event name). PostToolUse repairs the edited file (`replace_file_content`/`write_to_file`/`multi_replace_file_content`) on disk; unfixable files are left as written (no backup/revert, since PostToolUse can't message the agent). PreToolUse stashes the path as a fallback, and Stop runs the stale temp-dir sweep. Validated end-to-end against agy 1.0.9. Note: file writes the model routes through `run_command` (shell) bypass the edit-tool hooks, as with other editors.
