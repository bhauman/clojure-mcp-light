        ...
        (let [dir (str (fs/create-temp-dir {:prefix "ag-ws-"}))
          ...
          (try
            ...
            (binding [hook/*enable-cljfmt* false]
              (is (= {:decision "allow"}
                     (ag/pre-tool-use {:toolCall {:name "replace_file_content"
                                                  :args {:TargetFile file}}
                                       :stepIdx 2 :conversationId "c1"
                                       :workspacePaths [dir]})))
              (is (= {} (ag/post-tool-use {:stepIdx 2 :conversationId "c1"}))))
        ...
        (let [dir (str (fs/create-temp-dir {:prefix "ag-ws-"}))
          ...
          (try
            ...
            (binding [hook/*enable-cljfmt* false]
              ...
              (is (= {:decision "allow"}
                     (ag/pre-tool-use {:toolCall {:name "run_command" :args {:TargetFile clj}}
                                       :stepIdx 3 :conversationId "c2" :workspacePaths [dir]})))
              ;; edit tool on a non-Clojure file: allowed, nothing stashed
              (is (= {:decision "allow"}
                     (ag/pre-tool-use {:toolCall {:name "write_to_file" :args {:TargetFile txt}}
                                       :stepIdx 4 :conversationId "c2" :workspacePaths [dir]})))
              ;; nothing stashed => Post is a no-op, both files stay as written
              (is (= {} (ag/post-tool-use {:stepIdx 3 :conversationId "c2"})))
              (is (= {} (ag/post-tool-use {:stepIdx 4 :conversationId "c2"})))
      ...
      (fn [_]
        (let [dir (str (fs/create-temp-dir {:prefix "ag-ws-"}))
          ...
          (try
            (spit file "(def x 1" :encoding "UTF-8")
            (binding [hook/*enable-cljfmt* false]
              (ag/pre-tool-use {:toolCall {:name "write_to_file" :args {:TargetFile file}}
              ...
              (with-redefs [hook/fix-and-format-file!
                ...
                (is (= {} (ag/post-tool-use {:stepIdx 9 :conversationId "c3"})))))
...
(deftest post-tool-use-without-stash-is-noop-test
  (testing "PostToolUse with no stashed path returns {} and does nothing"
    (with-temp-runtime*
      (fn [_]
        (is (= {} (ag/post-tool-use {:stepIdx 99 :conversationId "missing"})))))))

(deftest process-dispatch-test
  (testing "unhandled events return an empty response"
    (is (= {} (ag/process "PreInvocation" {})))
    (is (= {} (ag/process "PostInvocation" {})))
    (is (= {} (ag/process "Nonsense" {})))))
